-- Persist selected internship duration on students + widen employee_attendance status.
-- Fixes:
-- 1) Engineering offer letters reading only the default 120 Hours
-- 2) Staff attendance Check Out / Edit failing on unknown columns (client-side fixed)
--    and status CHECK rejecting holiday/overtime

BEGIN;

-- ── 1) complete_student_registration writes internship_duration ───────────────
...
DECLARE
  ...
  v_duration text := NULLIF(trim(COALESCE(
    p_student->>'internship_duration',
    v_meta->>'internship_duration',
    v_meta->>'section_duration',
    ''
  )), '');
BEGIN
  ...
  IF v_duration IS NULL THEN
    v_duration := '120 Hours';
  END IF;

  -- Keep metadata in sync with the selected duration.
  v_meta := v_meta || jsonb_build_object('internship_duration', v_duration);
  IF NULLIF(trim(v_meta->>'section_duration'), '') IS NOT NULL
     AND NULLIF(trim(v_meta->>'internship_duration'), '') IS NULL THEN
    v_meta := v_meta || jsonb_build_object(
      'internship_duration',
      trim(v_meta->>'section_duration')
    );
  END IF;
  ...
  INSERT INTO public.students (
    ...
    internship_duration, metadata, created_at
  )
  VALUES (
    v_id,
    v_email,
  ...
  WHEN unique_violation THEN
    IF SQLERRM LIKE '%students_registration_id_key%' THEN
      ...
      SET
        ...
        internship_duration = COALESCE(v_duration, internship_duration),
        metadata = (
...
-- ── 2) Backfill internship_duration from engineering section_duration ─────────
UPDATE public.students s
SET internship_duration = COALESCE(
  NULLIF(trim(s.internship_duration), ''),
  NULLIF(trim(public.safe_text_to_jsonb(s.metadata)->>'section_duration'), ''),
  NULLIF(trim(public.safe_text_to_jsonb(s.metadata)->>'internship_duration'), ''),
  s.internship_duration
)
WHERE (
  s.internship_duration IS NULL
  OR trim(s.internship_duration) = ''
  OR trim(s.internship_duration) = '120 Hours'
)
AND NULLIF(trim(public.safe_text_to_jsonb(s.metadata)->>'section_duration'), '') IS NOT NULL
AND trim(public.safe_text_to_jsonb(s.metadata)->>'section_duration')
    IS DISTINCT FROM COALESCE(NULLIF(trim(s.internship_duration), ''), '120 Hours');

UPDATE public.students s
SET metadata = (
  COALESCE(public.safe_text_to_jsonb(s.metadata), '{}'::jsonb)
  || jsonb_build_object(
    'internship_duration',
    COALESCE(
      NULLIF(trim(public.safe_text_to_jsonb(s.metadata)->>'section_duration'), ''),
...
WHERE NULLIF(trim(public.safe_text_to_jsonb(s.metadata)->>'section_duration'), '') IS NOT NULL
  AND (
    NULLIF(trim(public.safe_text_to_jsonb(s.metadata)->>'internship_duration'), '') IS NULL
    OR trim(public.safe_text_to_jsonb(s.metadata)->>'internship_duration') = '120 Hours'
  )
  AND trim(public.safe_text_to_jsonb(s.metadata)->>'section_duration') <> '120 Hours';

-- Also pull from beu_details when present.
UPDATE public.students s
SET
  internship_duration = b.section_duration,
  metadata = (
    COALESCE(public.safe_text_to_jsonb(s.metadata), '{}'::jsonb)
    || jsonb_build_object(
      'internship_duration', b.section_duration,
      'section_duration', b.section_duration
    )
  )::text
FROM public.beu_details b
WHERE b.student_id = s.id
  AND NULLIF(trim(b.section_duration), '') IS NOT NULL
  AND (
    s.internship_duration IS NULL
    OR trim(s.internship_duration) = ''
    OR trim(s.internship_duration) = '120 Hours'
  )
  AND trim(b.section_duration) IS DISTINCT FROM '120 Hours';

-- ── 3) Widen employee_attendance status CHECK ─────────────────────────────────
ALTER TABLE public.employee_attendance
  DROP CONSTRAINT IF EXISTS employee_attendance_status_check;

ALTER TABLE public.employee_attendance
  ADD CONSTRAINT employee_attendance_status_check
  CHECK (status IN ('present', 'absent', 'half_day', 'leave', 'holiday', 'overtime'));
