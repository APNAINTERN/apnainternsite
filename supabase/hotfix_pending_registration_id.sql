-- Replace EZY/PENDING/{uuid} placeholders with real EZY/{year}/INT/{seq} ids (offer letter Letter Ref.).

CREATE OR REPLACE FUNCTION public.is_placeholder_registration_id(p_reg text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT coalesce(trim(p_reg), '') = ''
    OR trim(p_reg) ~* '^EZY/PENDING/'
    OR trim(p_reg) ~* '/INT/PENDING$';
$$;

CREATE OR REPLACE FUNCTION public.ensure_student_registration_id(p_user_id uuid DEFAULT auth.uid())
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF v_reg IS NOT NULL AND NOT public.is_placeholder_registration_id(v_reg) THEN
    RETURN v_reg;
  END IF;
  ...
  WHILE EXISTS (
    ...
    WHERE s.registration_id = v_reg AND s.id <> v_uid
  ) LOOP
    v_reg := public.allocate_next_registration_id(v_year);
  END LOOP;

  UPDATE public.students
  SET registration_id = v_reg
  WHERE id = v_uid;
...
REVOKE ALL ON FUNCTION public.ensure_student_registration_id(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ensure_student_registration_id(uuid) TO authenticated, anon;

-- Backfill all placeholder registration ids
DO $$
DECLARE
  r record;
  v_reg text;
...
BEGIN
  FOR r IN
    ...
    FROM public.students s
    WHERE public.is_placeholder_registration_id(s.registration_id)
    ORDER BY s.created_at NULLS LAST
  LOOP
    v_year := coalesce(extract(year FROM r.created_at)::integer, extract(year FROM now())::integer);
    v_reg := public.allocate_next_registration_id(v_year);
    WHILE EXISTS (
      SELECT 1 FROM public.students o
      WHERE o.registration_id = v_reg AND o.id <> r.id
    ) LOOP
      v_reg := public.allocate_next_registration_id(v_year);
    END LOOP;
    UPDATE public.students SET registration_id = v_reg WHERE id = r.id;
  END LOOP;
END $$;
...
BEGIN
  ...
  IF public.is_placeholder_registration_id(v_reg) THEN
    v_reg := public.allocate_next_registration_id(v_year);
    WHILE EXISTS (
  ...
  IF EXISTS (SELECT 1 FROM public.students s WHERE s.id = v_id) THEN
    ...
    SET
      ...
      registration_id = CASE
        WHEN public.is_placeholder_registration_id(registration_id) THEN v_reg
        WHEN registration_id IS NULL OR trim(registration_id) = '' THEN v_reg
        ELSE registration_id
      END,
      metadata = COALESCE(metadata, '{}'::jsonb) || v_meta
    WHERE id = v_id;

    RETURN jsonb_build_object('id', v_id::text, 'email', v_email, 'registration_id', v_reg);
  END IF;
  ...
  IF FOUND
    ...
    AND NOT public.is_placeholder_registration_id(v_legacy.registration_id)
    AND NOT EXISTS (
      SELECT 1
      FROM public.students s
  ...
  ON CONFLICT (id) DO UPDATE SET
    ...
    registration_id = CASE
      WHEN public.is_placeholder_registration_id(public.students.registration_id) THEN EXCLUDED.registration_id
