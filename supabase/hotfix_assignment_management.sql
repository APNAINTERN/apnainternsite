-- Assignment management: targeting, MCQ + long answer, student scoping, admin grading.
...
ALTER TABLE public.assignment_questions
  ...
  ADD COLUMN IF NOT EXISTS word_limit_min integer DEFAULT 500,
  ADD COLUMN IF NOT EXISTS word_limit_max integer DEFAULT 1000;
...
BEGIN
  ...
  WHERE public.student_matches_class_targets(s.id, p_universities, p_colleges, p_domains, NULL);
  RETURN v_count;
END;
$$;
...
BEGIN
  ...
  LOOP
    ...
    VALUES (
      ...
      COALESCE(NULLIF(trim(v_q->>'question_type'), ''), 'mcq'),
      CASE
        WHEN COALESCE(v_q->>'question_type', 'mcq') = 'long_answer' THEN '[]'::jsonb
        ELSE COALESCE(v_q->'options', '[]'::jsonb)
      END,
      CASE
        WHEN COALESCE(v_q->>'question_type', 'mcq') = 'long_answer' THEN NULL
        ELSE (v_q->>'correct_option_index')::integer
      END,
      GREATEST(1, COALESCE((v_q->>'marks')::integer, 1)),
      v_idx,
      COALESCE((v_q->>'word_limit_min')::integer, 500),
      COALESCE((v_q->>'word_limit_max')::integer, 1000)
    );
    v_idx := v_idx + 1;
  END LOOP;
  ...
  SET recipient_count = public.admin_count_assignment_targets(
        a.target_universities, a.target_colleges, a.target_domains
      )::integer,
      updated_at = now()
  WHERE a.id = v_id;
  RETURN v_id;
...
BEGIN
  ...
  WHERE a.is_active = true
    AND (
      public.class_targets_are_universal(a.target_universities, a.target_colleges, a.target_domains, NULL)
      OR public.student_matches_class_targets(
        auth.uid(), a.target_universities, a.target_colleges, a.target_domains, NULL
      )
      OR (
        a.target_universities IS NULL AND a.target_colleges IS NULL AND a.target_domains IS NULL
      )
    )
...
BEGIN
  ...
  IF NOT (
    public.class_targets_are_universal(v_assgn.target_universities, v_assgn.target_colleges, v_assgn.target_domains, NULL)
    OR public.student_matches_class_targets(
      v_uid, v_assgn.target_universities, v_assgn.target_colleges, v_assgn.target_domains, NULL
    )
    OR (v_assgn.target_universities IS NULL AND v_assgn.target_colleges IS NULL AND v_assgn.target_domains IS NULL)
  ) THEN
...
BEGIN
  ...
  LOOP
    IF v_q.question_type = 'long_answer' THEN
      ...
      IF v_text IS NULL OR length(v_text) < 10 THEN
        RAISE EXCEPTION 'Long answer required for all descriptive questions';
      END IF;
    ELSE
      BEGIN
...
REVOKE ALL ON FUNCTION public.admin_count_assignment_targets(text[], text[], text[]) FROM PUBLIC;
...
GRANT EXECUTE ON FUNCTION public.admin_count_assignment_targets(text[], text[], text[]) TO authenticated;
