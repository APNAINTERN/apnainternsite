-- Run in Supabase SQL Editor if college registration blocking was applied earlier.
-- Clears SRKG date gates and restores complete_student_registration without window checks.

UPDATE public.colleges c
SET registration_start_date = NULL,
    registration_end_date = NULL
FROM public.universities u
WHERE c.university_id = u.id
  AND u.name ILIKE '%babasaheb%bhimrao%ambedkar%'
  AND c.name ILIKE '%s.r.k.g%'
  AND c.name ILIKE '%sitamarhi%';

UPDATE public.colleges
SET registration_start_date = NULL,
    registration_end_date = NULL
WHERE id = '10bd4d19-518a-4199-8825-63c72d9a3d9c'
  AND university_id = 'f822dcc6-b4ce-4c3e-82e7-e6f33d16df43';
