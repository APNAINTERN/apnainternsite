-- Admin / staff: create minimal paid student (auth + students + payment_success) from portal UI.
...
REVOKE ALL ON FUNCTION public.admin_create_minimal_student_registration(text, text, text, text, text, bigint) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.admin_create_minimal_student_registration(text, text, text, text, text, bigint) TO authenticated;
