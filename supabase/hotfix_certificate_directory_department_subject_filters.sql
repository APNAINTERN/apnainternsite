-- Certificate directory filters: department + subject (Download & verify tab).
-- Run in Supabase SQL Editor after hotfix_certificate_directory_pagination.sql.
...
BEGIN
  ...
  RETURN (
    ...
    AND (
      ...
      OR trim(COALESCE(s.department, s.metadata->>'department', '')) = ANY (p_departments)
    )
    AND (
      p_subjects IS NULL
      OR cardinality(p_subjects) = 0
      OR trim(COALESCE(s.subject, s.metadata->>'subject', '')) = ANY (p_subjects)
    )
    AND (
      p_domain IS NULL
      OR p_domain = ''
...
REVOKE ALL ON FUNCTION public.admin_count_certificates_directory(text, text[], text[], text, text, text[], text[]) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.admin_list_certificates_directory(integer, integer, text, text[], text[], text, text, text[], text[]) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION public.admin_count_certificates_directory(text, text[], text[], text, text, text[], text[]) TO authenticated;
GRANT EXECUTE ON FUNCTION public.admin_list_certificates_directory(integer, integer, text, text[], text[], text, text, text[], text[]) TO authenticated;
