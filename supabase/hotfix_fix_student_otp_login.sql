-- Fix student OTP login ("Could not verify the login code" / error internal).
-- Run this ENTIRE file once in Supabase SQL Editor, then wait ~30s.
-- Prerequisites: pgcrypto + password_resets (supabase/custom_otp_reset.sql or hotfix_student_otp_login.sql)

CREATE EXTENSION IF NOT EXISTS pgcrypto WITH SCHEMA extensions;

-- Safe identity insert (duplicate provider rows must not break OTP login)
...
BEGIN
  ...
  SELECT EXISTS (
    ...
    WHERE c.table_schema = 'public'
      AND c.table_name = 'password_resets'
