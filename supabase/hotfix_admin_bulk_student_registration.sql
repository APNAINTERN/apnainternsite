-- Bulk upload: tag students with metadata.source = admin_bulk_upload
-- Run in Supabase SQL Editor after admin_create_minimal_student_registration is deployed.
...
REVOKE ALL ON FUNCTION public.admin_create_minimal_student_registration(text, text, text, text, text, bigint, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.admin_create_minimal_student_registration(text, text, text, text, text, bigint, text) TO authenticated;
