...
export type NonEngineeringUniversityConfig = {
  id: string;
...
export type NonEngineeringConfigInput = {
...
function isMissingTable(err: { code?: string; message?: string } | null): boolean {
  ...
  return err.code === "42P01" || (msg.includes("non_engineering_university_configs") && msg.includes("does not exist"));
}
...
function rowToConfig(
  ...
  universityName?: string
): NonEngineeringUniversityConfig {
  return {
    id: String(row.id || ""),
...
export function defaultNonEngineeringOptions(): Pick<
  NonEngineeringUniversityConfig,
  "courses" | "branches_by_course" | "domains"
> {
  const courses = withOtherOption(["B.A.", "B.Sc", "B.Com", "M.A.", "M.Sc", "M.Com"]);
  const branches_by_course: Record<string, string[]> = {};
  for (const course of courses) {
    branches_by_course[course] = withOtherOption(["Other"]);
  }
  return {
    courses,
    branches_by_course,
...
export function resolveNonEngineeringOptions(
  config: NonEngineeringUniversityConfig | null | undefined
): {
  courses: string[];
  branchesByCourse: Record<string, string[]>;
  domains: string[];
} {
  const fallback = defaultNonEngineeringOptions();
  const courses =
  ...
  for (const course of courses) {
    ...
    branchesByCourse[course] =
      fromConfig?.length ? withOtherOption(fromConfig) : withOtherOption(["Other"]);
  }
  return {
    courses,
    branchesByCourse,
    // Non-tech always uses global internship domains
    domains: [],
  };
}

export async function fetchAllNonEngineeringConfigs(
  client: SupabaseClient
): Promise<NonEngineeringUniversityConfig[]> {
  const { data, error } = await client
    .from("non_engineering_university_configs")
    .select("*")
...
export async function fetchNonEngineeringConfigMap(
  client: SupabaseClient
): Promise<Map<string, NonEngineeringUniversityConfig>> {
  const rows = await fetchAllNonEngineeringConfigs(client);
  const map = new Map<string, NonEngineeringUniversityConfig>();
...
export async function fetchNonEngineeringConfigForUniversity(
  client: SupabaseClient,
  universityId: string
): Promise<NonEngineeringUniversityConfig | null> {
  const { data, error } = await client
    .from("non_engineering_university_configs")
    .select("*")
...
async function ensureColleges(
  ...
  const afterMatched = names.filter((n) =>
    after.some(
      (c) => c.name.toLowerCase() === n.toLowerCase() || institutionsMatch(c.name, n)
    )
  );
  if (afterMatched.length < names.length) {
  ...
  const needsFeeSync = after.filter((c) => {
    ...
    if (!listed && !nameSet.has(c.name.toLowerCase())) return false;
...
export async function saveNonEngineeringConfig(
  client: SupabaseClient,
  input: NonEngineeringConfigInput
): Promise<
  NonEngineeringUniversityConfig & {
...
> {
  ...
  const courseNames = input.courses.map((c) => c.trim()).filter((c) => c && c !== "Other");
  const courses = courseNames.length > 0 ? withOtherOption(courseNames) : [];
...
export async function deleteNonEngineeringConfig(
  client: SupabaseClient,
  configId: string
): Promise<void> {
  const { error } = await client.from("non_engineering_university_configs").delete().eq("id", configId);
  if (error && !isMissingTable(error)) throw error;
}

/**
 * Parse Non-Eng. Management textareas: one entry per line.
